...
import { FaArrowLeft, FaMapMarkerAlt, FaUtensils, FaStar, FaPlus, FaPhone, FaGlobe, FaClock, FaDollarSign, FaHeart, FaShare, FaChevronRight } from "react-icons/fa";
...
export default function RestaurantDetailPage() {
  ...
  const allPhotos = useMemo(() => {
    const photos: string[] = [];
    if (restaurant?.image) photos.push(restaurant.image);
            ...
            {allPhotos.length > 1 && (
              ...
              >
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl font-bold text-white">Photos ({allPhotos.length})</h2>
                  <button className="text-sm text-orange-500 font-semibold hover:text-orange-400 transition-colors flex items-center gap-1">
                    View All
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {allPhotos.slice(0, 6).map((photo, idx) => (
                    <motion.div
                      ...
                      className="aspect-square rounded-2xl overflow-hidden border border-white/10 hover:border-orange-500/50 transition-all cursor-pointer group"
                    >
                      <CloudImage
                        src={photo}
                        alt={`Photo ${idx + 1}`}
